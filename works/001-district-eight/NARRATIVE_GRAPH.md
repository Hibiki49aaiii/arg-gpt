# 第八避難区 — Narrative Graph

## Narrative Spine

Trailhead → 欠番08 → 古いPDF → 二重住所 → 1998-08-14 → 33秒音声 → 水城結の日記 → 共同記憶 → 桐谷研究ノート → 三枝の再評価 → 地図完成 → 調査=再構築 → Ending

## Player Theory Progression

| Act | Intended Player Theory | Later Status |
|---|---|---|
| 0 | URLの欠番 | Wrong |
| 1 | 昔存在した地区 | Partly Wrong |
| 2 | 隠された隔離施設 | Partly True |
| 3 | 秘密放送による事件 | Misleading |
| 4 | 集団催眠・認知実験 | Wrong mechanism |
| 5 | 記憶災害を封じるための隠蔽 | Mostly True |
| 6 | 自分の調査が再発原因 | True |

## Core Evidence

### EV-001 防災区一覧
Visible: 第1〜第7避難区。
Hidden: asset命名規則に08。
Function: Rabbit Hole。

### EV-002 平成10年度給水計画
Visible: 「第八避難区 旧八号集会所」。
Contradiction: 現行資料に第八区はない。

### EV-003 学校配布物
Visible: 第八避難区の児童へ別行動指示。
Hidden Meaning: 対象は場所ではなく人。

### EV-004 住民名簿断片
Visible: 水城結 / 第八避難区。

### EV-005 卒業アルバム
Visible: 同じ水城結が東三丁目在住。
Reveal: 二重住所。

### EV-006 防災無線送信ログ
Visible: 1998-08-14 18:00 試験放送のみ。

### EV-007 受信側カセット
Visible: 33秒余分。
Reveal: 送信側にない音声。

### EV-008 水城結の日記
Visible: 水無坂の家の記憶と母親との住所認識の食い違い。
Reveal: 本人にとって架空記憶が本物。

### EV-009 児童画コレクション
Visible: 複数の子どもが同じ公園、坂、集会所を描く。
Reveal: 同一の架空空間を共有。

### EV-010 古地図比較
Visible: 1998年以前には水無坂がなく、後年資料ほど道路らしき線が現れる。

### EV-011 三枝内部告発メモ
Visible: 「人を消している」。
First Reading: 行政が被害者を隠蔽。
Second Reading: 人名と場所の検索可能性を消していた。

### EV-012 桐谷研究ノート
Visible: 記述密度と再現率が相関。
Reveal: 記録が媒介。

### EV-013 三枝最終メモ
Visible: 「残せば戻る。消せば彼らが消える。」
Function: Final moral problem。

### EV-014 復元地図
Player-created。
Function: プレイヤー自身が最後の危険なEvidenceを作る。

## Major Puzzles

### PZ-001 URL / Asset Sequence
01〜07の規則と複数ヒントから08の欠落を見つける。
Reveal: 第八区の存在痕跡。

### PZ-002 PDF Metadata
複数PDFのmetadata、source filename、creation dateを比較。
Reveal: area08が意図的に削除された可能性。

### PZ-003 Two Addresses
住民名簿、卒業アルバム、学区資料を生年月日で照合。
Reveal: 水城結が同時期に2住所。

### PZ-004 33-second Audio
複数の受信録音と時報を基準に欠損音声を復元。
Reveal: 送信ログにない放送。
Accessibility: 音声スペクトログラムだけを必須解法にしない。

### PZ-005 Diary Pagination
天気、学校行事、テレビ欄などから日記断片を時系列復元。
Reveal: 存在しない家への郷愁。

### PZ-006 Children's Map
児童画の共通ランドマークを抽出し相対位置を求める。
Reveal: 独立した人物が同一空間を共有。

### PZ-007 Archive Overlay
年代の違う地図の基準点を合わせる。
Reveal: 後年資料ほど水無坂が形成される。

### PZ-008 Research Index
削除ページ番号と引用ページを逆引き。
Reveal: 記録密度が現象を強める。

### PZ-009 Final Reconstruction
全Actの地名、距離、ランドマークで地図を完成。
Consequence: 第八区ページが出現。
Reveal: Puzzle solved = anomaly reconstructed。

## Site Architecture

1. SITE-001 旧久代市防災情報ミラー — Trailhead
2. SITE-002 久代市史料デジタルアーカイブ — 地図・広報・議事録
3. SITE-003 久代市学校史資料室 — 水城結・学校資料・児童資料
4. SITE-004 個人サイト「記録庫・三枝」 — 内部告発と後半再文脈化
5. SITE-005 地域ラジオ保存会 — 1998年音声資料
6. SITE-006 研究資料ミラー — 桐谷研究ノート
7. SITE-007 現在の久代市防災ページ — 現実改変の指標

## Recovery Routes

必須進行は単一路線にしない。
- 二重住所へ: 給水計画→学校 / 三枝ページ→水城姓 / 古地図→学区番号
- 音声へ: 防災ログの日付 / 日記の日付 / 地域ラジオ周年記事
- 研究ノートへ: 三枝引用 / ラジオインタビュー / 市議会資料

## Red Herrings

### RH-001 行政による人体実験
もっともらしいが、異常記憶は研究開始前から存在。

### RH-002 片瀬が秘密放送を流した
当日勤務だが、送信機側記録に音声がない。

### RH-003 第八区で大事故が起きた
避難・隔離・欠番は事実だが、第八区は事件後に行政コード化。

## Final Recontextualizations

- 三枝は資料を残した英雄 → 最後には資料を消そうとした
- 市は人を消した → 情報伝播も止めようとしていた
- 第八区を探す → 第八区を作る
- 欠けた地図を完成させる → 欠けていたこと自体が封印だった