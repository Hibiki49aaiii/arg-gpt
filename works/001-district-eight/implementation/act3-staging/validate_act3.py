...
# Pre-alignment transcript display must not trivially expose full solved terms.
for tape in (a,b):
...
for forbidden in data.get("player_facing_forbidden_terms",[]):
    # Author contract can list forbidden terms, so only inspect evidence/composite player data.
